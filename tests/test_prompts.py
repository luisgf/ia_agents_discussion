...
def test_round1_keeps_initial_history_drops_current_agents() -> None:
    history = [_msg("user", "initial summary context")] + _ROUND1[:3]
    out = _history_before_current_round(history, 1)
    assert [m.role for m in out] == ["user"]


def test_round2_cuts_at_first_moderator_keeps_user_comments() -> None:
    history = _ROUND1 + [_msg("user", "HITL comment"), _msg("diagnostic_agent", "diag R2")]
    ...
    assert out[-1].content == "HITL comment"
...
def test_rebuttal_role_literal_is_filtered() -> None:
    history = [_msg("diagnostic_rebuttal", "rebuttal in progress")]
...
def test_history_mode_round1_full() -> None:
    assert _history_mode(_state(history_summary="summary")) == ("", "full")


def test_history_mode_round2_with_summary_compressed() -> None:
    assert _history_mode(_state(round=2, history_summary="summary")) == ("summary", "compressed")


def test_history_mode_compress_disabled() -> None:
    assert _history_mode(_state(round=3, history_summary="summary", compress_history=False)) == ("", "full")
...
def test_prompt_history_compressed_keeps_only_user_tail_for_skeptic() -> None:
    history = _ROUND1 + [_msg("user", "comment"), _msg("diagnostic_agent", "diag R2")]
    state = _state(history=history, round=2, history_summary="summary")
...
def test_format_history_compressed_with_empty_tail_shows_summary() -> None:
    out = format_history([], history_summary="the summary", mode="compressed")
    assert "the summary" in out
    assert "Sin historial previo" not in out


def test_format_history_compressed_renders_tail() -> None:
    out = format_history([_msg("user", "new data")], history_summary="summary", mode="compressed")
    assert "summary" in out
    assert "new data" in out


# ── summarize_history: fix A (round already incremented) + accumulation ──
...
class _FakeSummaryModel:
    ...
    def invoke(self, messages: list) -> object:
        ...
        return type("R", (), {"content": "GENERATED-SUMMARY", "usage_metadata": {}})()
...
def test_summarize_runs_after_round1_continue(monkeypatch) -> None:
    ...
    # The moderator already incremented round to 2 with status continue
    state = _state(history=list(_ROUND1), round=2, moderator_model="m", summary_model="m")
    result = summarize_history(state)
    assert result.get("history_summary") == "GENERATED-SUMMARY"
...
def test_summarize_is_cumulative(monkeypatch) -> None:
    ...
    assert "diag R1" not in prompt, "only the new messages go in raw"
